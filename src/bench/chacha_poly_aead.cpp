...
static ChaCha20Poly1305OpenSSHAEAD aead_openssh(k1, 32, k2, 32);
...
static void CHACHA20_POLY1305_OPENSSH_AEAD(benchmark::State& state, size_t buffersize, bool include_decryption)
{
    std::vector<unsigned char> in(buffersize + CHACHA20_POLY1305_AEAD_AAD_LEN + POLY1305_TAGLEN, 0);
    std::vector<unsigned char> out(buffersize + CHACHA20_POLY1305_AEAD_AAD_LEN + POLY1305_TAGLEN, 0);
    uint64_t seqnr_payload = 0;
    ...
    while (state.KeepRunning()) {
        // encrypt or decrypt the buffer with a static key
        assert(aead_openssh.Crypt(seqnr_payload, seqnr_aad, aad_pos, out.data(), out.size(), in.data(), buffersize, true));

        if (include_decryption) {
            // if we decrypt, include the GetLength
            assert(aead_openssh.GetLength(&len, seqnr_aad, aad_pos, in.data()));
            assert(aead_openssh.Crypt(seqnr_payload, seqnr_aad, aad_pos, out.data(), out.size(), in.data(), buffersize, true));
        }
...
static void CHACHA20_POLY1305_OPENSSH_AEAD_64BYTES_ONLY_ENCRYPT(benchmark::State& state)
{
    CHACHA20_POLY1305_OPENSSH_AEAD(state, BUFFER_SIZE_TINY, false);
}

static void CHACHA20_POLY1305_OPENSSH_AEAD_256BYTES_ONLY_ENCRYPT(benchmark::State& state)
{
    CHACHA20_POLY1305_OPENSSH_AEAD(state, BUFFER_SIZE_SMALL, false);
}

static void CHACHA20_POLY1305_OPENSSH_AEAD_1MB_ONLY_ENCRYPT(benchmark::State& state)
{
    CHACHA20_POLY1305_OPENSSH_AEAD(state, BUFFER_SIZE_LARGE, false);
}

static void CHACHA20_POLY1305_OPENSSH_AEAD_64BYTES_ENCRYPT_DECRYPT(benchmark::State& state)
{
    CHACHA20_POLY1305_OPENSSH_AEAD(state, BUFFER_SIZE_TINY, true);
}

static void CHACHA20_POLY1305_OPENSSH_AEAD_256BYTES_ENCRYPT_DECRYPT(benchmark::State& state)
{
    CHACHA20_POLY1305_OPENSSH_AEAD(state, BUFFER_SIZE_SMALL, true);
}

static void CHACHA20_POLY1305_OPENSSH_AEAD_1MB_ENCRYPT_DECRYPT(benchmark::State& state)
{
    CHACHA20_POLY1305_OPENSSH_AEAD(state, BUFFER_SIZE_LARGE, true);
}
...
BENCHMARK(CHACHA20_POLY1305_OPENSSH_AEAD_64BYTES_ONLY_ENCRYPT, 500000);
BENCHMARK(CHACHA20_POLY1305_OPENSSH_AEAD_256BYTES_ONLY_ENCRYPT, 250000);
BENCHMARK(CHACHA20_POLY1305_OPENSSH_AEAD_1MB_ONLY_ENCRYPT, 340);
BENCHMARK(CHACHA20_POLY1305_OPENSSH_AEAD_64BYTES_ENCRYPT_DECRYPT, 500000);
BENCHMARK(CHACHA20_POLY1305_OPENSSH_AEAD_256BYTES_ENCRYPT_DECRYPT, 250000);
BENCHMARK(CHACHA20_POLY1305_OPENSSH_AEAD_1MB_ENCRYPT_DECRYPT, 340);
